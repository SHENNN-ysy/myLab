...
MyBlog 是一个前后端分离的个人博客系统，包含访客博客、管理后台和 Spring Boot API，支持本地 Docker 一键部署，以及 Jenkins + 私有 Registry 的生产 CI/CD。
...
修改 `.env` 中的数据库密码、Redis 密码、JWT 密钥、初始管理员和 OSS 配置。后台路径由 `ADMIN_ROUTE` 控制，默认 `/admin`；前端 API 默认使用同源的 `/api/v1`。
...
本地一键部署只编译并打包应用，不执行后端单元测试或 Testcontainers 集成测试；完整质量门由 CI 执行。
...
分别启动应用（需要 Node.js 20+、JDK 21 和 Maven）：
...
开发端口为后端 `8000`、前台 `5173`、后台 `5174`、PostgreSQL `15432`、Redis `6379`。两个前端开发服务器都会把 `/api` 代理到后端 `8000`；后台构建时通过 `VITE_ADMIN_ROUTE` 生成资源和路由基础路径。
...
从空白服务器开始的 Docker、Registry、Jenkins、域名、证书、Webhook、Job 和首次发布步骤见 [生产部署指南](deploy/README.md)。

## 项目结构

```text
MyBlog/
├── myblog/                         # React 博客前台 SPA
├── admin/                          # React 管理后台 SPA
...
├── scripts/                        # Registry 运维脚本
├── docker-compose.yml              # 本地一键部署
├── Jenkinsfile.ci                  # 质量门与 master 镜像构建
├── Jenkinsfile.cd                  # 指定镜像 tag 部署
...
| [myblog/README.md](myblog/README.md) | 博客前台开发说明 |
| [admin/README.md](admin/README.md) | 管理后台功能与开发说明 |
