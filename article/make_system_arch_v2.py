# -*- coding: utf-8 -*-
"""绘制 MyBlog 系统架构图 v2（前端已重构为 React 技术栈）。"""
import sys
...
def main() -> None:
    ...
    # ===== 界面层（React 重构后）=====
    container_with_rows(ax, LX, 104, 65, 20, "博客前台（React 18 + TypeScript）",
                        [["GSAP 动效", "Tailwind CSS 4"], ["React Router / Zustand", "七大内容模块 + MyLab"]],
                        C_VIEW, inner_fs=9.5)
    container_with_rows(ax, 83, 104, 65, 20, "管理后台（React 18 + TypeScript）",
                        [["Ant Design 5", "ECharts 5"], ["Axios / Zustand", "版本化内容与文件管理"]],
                        C_VIEW, inner_fs=9.5)
