...
def main() -> None:
    fig, ax = plt.subplots(figsize=(17.5, 13.2))
    ax.set_xlim(0, 190)
    ax.set_ylim(0, 132)
    ax.axis("off")

    ax.text(81, 129.5, "MyBlog 后端架构图", ha="center", va="center",
            fontsize=19, fontweight="bold", color="#1a2634")
    ax.text(170, 127, "职责说明", ha="center", va="center",
    ...
    for name, y0, y1 in layers:
        ...
        # 右侧职责说明：与层同色系的浅底注释框
        note = LAYER_NOTES[name]
        ax.text(152, (y0 + y1) / 2, note, ha="left", va="center",
                fontsize=10.5, color="#4a5568", linespacing=1.7)
        # 层与说明之间的虚线引导
        ax.plot([149.5, 151.5], [(y0 + y1) / 2, (y0 + y1) / 2],
                color="#aab6c2", lw=1.2, ls="--")
