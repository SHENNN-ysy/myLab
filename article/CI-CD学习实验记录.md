...
> 本文记录 MyBlog 从“登录服务器拉代码并现场构建”演进到“GitHub PR + Jenkins + 私有 Registry + 不可变镜像部署”的过程。作为一次CI/CD的学习实践，我想从中学习到 Jenkinsfile 的语法以及流水线的搭建，并配合代码仓库的hook和私有镜像仓库 Registry，完成CI/CD的解耦以及项目的生产级部署。在这个过程中本文会说明为什么要改、每一层解决了什么问题、实验中踩过哪些坑，以及新方案还存在哪些边界。
...
运行时还依赖 PostgreSQL、Redis 和 Nginx。项目早期规模不大时，我采用了一种最直接的部署方式：在服务器保留一份 Git 工作区，更新时执行 `git pull`，然后让 Docker Compose 从当前源码重新构建并启动所有服务。
...
这套方案在项目早期确实很省心：概念少、搭建快，一个 Compose 文件就能完成数据库、缓存、后端、前端和网关的编排。但随着我想给项目加上 HTTPS、让后台走独立路由、尝试引入回滚能力，我慢慢发现"能部署"和"能可控地发布"是两回事。
...
这个流程的核心特征是：**源码、构建、部署和运行环境全部耦合在生产服务器上**。接下来我想从三个角度说明，为什么这个"简单粗暴"的方式开始让我想要寻找新的解法。

## 2. 为什么要从旧方案切换出去

旧方案没有绝对的错误，只是我想实践一下CI/CD应该怎么做，能做到什么.....

### 2.1 流水线是怎么搭建的？jenkins怎么使用？
...
### 2.2 我的整个开发流程缺少了独立的测试环境，这个能否自动化？
