...
/**
 * Spring Security 配置：Redis Bearer 会话认证、公开接口白名单、统一 401/403 JSON 响应及 CORS。
 *
 * <p>设计要点：SessionAuthenticationFilter 只负责校验会话并在 SecurityContext 中建立身份，
...
public class SecurityConfig {
    ...
    /**
     * 构建安全过滤链：关闭 CSRF、禁用 Servlet Session，放行健康检查/登录/Swagger 及公开接口，
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, SessionAuthenticationFilter sessionFilter,
        ...
        return http
                .csrf(csrf -> csrf.disable()) // Bearer 令牌不使用 Cookie 传输，无需 CSRF 防护
                .cors(cors -> {})
