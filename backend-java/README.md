...
五个模块 `skills`、`footprints`、`hobbies`、`vibe`、`mylab` 使用关系表保存完整版本。`content_releases` 维护 `DRAFT/PUBLISHED/ARCHIVED/OFFLINE` 状态；发布内容只读，继续编辑或恢复历史会创建更高版本号的新草稿。
...
数据库最终结构共 13 张业务表。V1 直接创建完整表结构，V2 创建五个初始草稿和五个全局标签。项目不启用 Flyway；Docker 只在 PostgreSQL 空数据卷首次初始化时执行 V1/V2。当前不包含访问量、访客数、点赞数、`support` 或独立 `projects` 模块。
